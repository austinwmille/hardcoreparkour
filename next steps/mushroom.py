...
# Note: This function no longer requires a categories dictionary.
def generate_news_metadata(file_path):
    ...
    try:
        response = client.chat.completions.create(
            model="o1-mini",  # Replace "o1" with your intended model (e.g., "gpt-4" or "gpt-3.5-turbo")
            messages=[
                {
                    "role": "system", 
...
def upload_video(youtube, file_path, metadata):
    ...
    # Use metadata description if available, else read from description.txt
...
# Create a custom thumbnail by overlaying a black box with text (e.g., "part 1") on the base thumbnail image.
def create_thumbnail(base_thumbnail_path, part_text, output_path):
    ...
    # Define the black box dimensions and position
    box_width, box_height = 200, 50
    box_x = (width - box_width) // 2
    box_y = height // 2 + 20  # Just below the middle

    draw = ImageDraw.Draw(img)
    black_color = (0, 0, 0, 200)  # Black with opacity
    draw.rectangle([box_x, box_y, box_x + box_width, box_y + box_height], fill=black_color)

    text = part_text  # e.g., "part 1"
    font_size = 28  # Larger font size for better visibility
    try:
        # Attempt to load a bold version of Arial
    ...
    # Draw the text in white on top of the black box.
...
def process_folder(youtube, folder_path):
    # Look for a base thumbnail file (.jpg) in the folder.
    ...
    # Create a sorted list of video files (ignoring subfolders)
    video_files = sorted(
    ...
    for file_path in video_files:
        ...
        if metadata.get("title") != "Untitled Video":
            ...
            if base_thumbnail_file and video_id:
                ...
                # Optionally, delete the temporary thumbnail file
